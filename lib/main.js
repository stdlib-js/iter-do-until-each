...
* // ...
*/
function iterDoUntilEach( iterator, predicate, fcn, thisArg ) {
	var iter;
	var FLG;
	var i;
	if ( !isIteratorLike( iterator ) ) {
		throw new TypeError( format( '1tJ46', iterator ) );
	}
	if ( !isFunction( predicate ) ) {
		throw new TypeError( format( '1tJ2H', predicate ) );
	}
	if ( !isFunction( fcn ) ) {
		throw new TypeError( format( '1tJ3N', fcn ) );
	}
	i = -1;
	...
	*/
	function factory() {
		return iterDoUntilEach( iterator[ iteratorSymbol ](), predicate, fcn, thisArg ); // eslint-disable-line max-len
	}
}
...
module.exports = iterDoUntilEach;
